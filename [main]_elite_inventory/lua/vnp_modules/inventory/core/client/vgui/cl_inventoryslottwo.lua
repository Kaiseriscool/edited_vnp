/*
    just here so that the inventory won't be funky with other stuff :c
 */
...
function PANEL:Paint(w,h)
    ...
    surface.DrawRect(c, c, w - c * 2, h - c * 2)
    ...
    if !hovered && self.lerp < 1 then return end

    self.lerp = Lerp(0.025, self.lerp, hovered && 20 or 0)
...
function PANEL:Think()
    ...
    if self.Data and (!self.Model or !IsValid(self.Model)) then
       ...
       --print(itemData.Class , "Class btw")
       // WHY ARE THERE 2 FUCKING FILES
...
vgui.Register( "VNPInvSlot2", PANEL, "DButton")